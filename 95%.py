...
screen_title = 'TURTLE DRAG RACE'  # The title
t1_border = 'SkyBlue'  # turtle 1 color
t1_fill = 'Black'
t2_border = 'Pink'  # turtle 2 color
t2_fill = 'Black'
BgColor = 'Black'  # Screen Background Color
distance = 250  # coordinate of the distance from the start to finish line
W_tur = 650  # width of the screen using the variable W_tur
H_tur = 180  # height of the screen using the variable H_tur
turtle_shape = 'turtle'

# Screen layout
display = turtle.Screen()
display.setup(W_tur, H_tur)
...
for i in range(5):
    fin_line.goto(distance, (85 - (i * 20 * 2)))
    fin_line.stamp()

for j in range(4):
    fin_line.goto(distance + 20, ((85 - 20) - (j * 20 * 2)))
    fin_line.stamp()
...
if T1.xcor() >= distance and T2.xcor() >= distance:
    ...
    if bet == name1:
        ...
        color = 'Green'
    elif bet != name1 and name2:
        winner_message = "YOU BET WHO?! Please Try Again."
        color = 'Red'
    else:
...
result.goto(0, -15)
result.color(color)
result.write(winner_message, align="center", font=("Courier", 14, "normal"))
result.hideturtle()
